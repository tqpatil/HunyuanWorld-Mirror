...
def render_incremental_splats(
    ...
    for ply_path in ply_files:
        ...
        V = cam_poses.shape[0]
        chunk_size = 20000  # Adjust based on your GPU memory
        ...
        for v in range(V):
            ...
            for start in range(0, N_total, chunk_size):
                ...
                if rgb_accum is None:
                    rgb_accum = render_colors[0, 0]
                    depth_accum = render_depths[0, 0, :, :, 0]
                    alpha_accum = render_alphas[0, 0, :, :, 0]
                else:
                    # Alpha compositing for RGB and depth
                    rgb_accum = rgb_accum * (1 - render_alphas[0, 0, :, :, 0]) + render_colors[0, 0] * render_alphas[0, 0, :, :, 0]
                    depth_accum = depth_accum * (1 - render_alphas[0, 0, :, :, 0]) + render_depths[0, 0, :, :, 0] * render_alphas[0, 0, :, :, 0]
                    alpha_accum = alpha_accum + render_alphas[0, 0, :, :, 0]
