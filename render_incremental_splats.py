...
output_root = Path("path/to/output")  # Update this path

# Find all incremental_splats directories
for root, dirs, files in os.walk(output_root):
    for d in dirs:
        if d == "incremental_splats":
            inc_dir = Path(root) / d
            ...
            for ply_file in sorted(inc_dir.glob("splats_views_0to*.ply")):
                ...
                H, W = 518, 518  # Or load from input frames if needed
                ...
                sh = splats["sh"].unsqueeze(0) if splats["sh"].ndim == 3 else splats["sh"]
                # Render each view
                render_colors, render_depths, _ = gs_renderer.rasterizer.rasterize_batches(
                    means, quats, scales, opacities,
                    sh if "sh" in splats else splats.get("colors"),
                    cam_poses_torch, cam_intrs_torch,
                    width=W, height=H,
